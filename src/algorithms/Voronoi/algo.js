...
var Voronoi = {
    ...
    if (r) {
      ...
      while (l < r) {
        ...
        if (c > 0) {
          ...
          return; /*Duplicate sites not allowed, quietly ignored*/
        }
      }
      q.splice(l, 0, o);
    } else {
  ...
  drawSites: function(ctx) {
    ...
    for (var iSite = 0; iSite < nSites; iSite++) {
      ...
      if (queueIsEmpty) {
        ctx.rect(site.x - 0.25, site.y - 0.25, 3, 3);
      } else {
        ctx.rect(site.x - 0.5, site.y - 0.5, 4, 4);
      }
    }
    ctx.globalAlpha = 1;
    ctx.fillStyle = '#000';
